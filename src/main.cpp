	...
	if (strcmp(page, "appointments") == 0) {
			...
			for (int i = 0; i < appointmentAmount; i++) {
				if (user.id == appointments[i].userId) {
					std::cout	<< "<tr>"
								...
								<< "\t\t\t<input type=\"hidden\" name=\"description\" value=\"" << appointments[i].description << "\" required>"
								<< "\t\t\t<input type=\"hidden\" name=\"time\" value=\"" << appointments[i].time << "\"  required>"
								<< "\t\t\t<input type=\"hidden\" name=\"date\" value=\"" << appointments[i].date << "\"  required>"
		...
		if (!appointmentEditError) {
			...
			std::cout << "<div class='card-body'>"
				...
				<< "\t\t\t<label>Art</label>"
				<< "\t\t\t<input type='text' class='form-control' id='art' name='description' value='" << appointmentDescription << "' maxlength='100' required>"
